# APIHighLatencyP95 Runbook

## Symptom

SignalCart API p95 latency is above the alert threshold.

## Impact

Requests may complete slowly even when they do not fail.
...
```bash
curl -G -s http://127.0.0.1:9090/api/v1/query   --data-urlencode 'query=histogram_quantile(0.95, sum by (le) (rate(signalcart_http_request_duration_seconds_bucket[5m])))' | jq .
```
...
```text
recover active simulation or stop load generation
```
