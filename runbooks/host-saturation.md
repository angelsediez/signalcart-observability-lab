# HostHighCPU / HostLowMemory Runbook

## Symptom

Host CPU or memory saturation alert is active.

## Impact

The local runtime may become slow or unstable.
...
```bash
curl -G -s http://127.0.0.1:9090/api/v1/query   --data-urlencode 'query=100 * (1 - avg(rate(node_cpu_seconds_total{mode="idle"}[5m])))' | jq .
```
...
```text
stop load generation and inspect docker stats
```
