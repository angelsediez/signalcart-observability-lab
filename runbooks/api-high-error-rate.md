# APIHighErrorRate Runbook

## Symptom

SignalCart API 5xx error percentage is elevated.

## Impact

Users or synthetic clients may receive server errors.
...
```bash
curl -G -s http://127.0.0.1:9090/api/v1/query   --data-urlencode 'query=sum(rate(signalcart_http_requests_total{status_code=~"5.."}[5m]))' | jq .
```
...
```text
recover active simulation or restart api after evidence capture
```
