# APIScrapeDown Runbook

## Symptom

Prometheus cannot scrape SignalCart API metrics.

## Impact

Prometheus loses application metrics and Grafana API panels may stop updating.
...
```bash
curl -G -s http://127.0.0.1:9090/api/v1/query   --data-urlencode 'query=up{job="signalcart-api"}' | jq .
```
