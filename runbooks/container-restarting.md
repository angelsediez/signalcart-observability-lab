# ContainerRestarting Runbook

## Symptom

A container restart was detected.

## Impact

A restart may indicate crashes, health check failures, configuration errors, or resource pressure.
...
```bash
curl -G -s http://127.0.0.1:9090/api/v1/query   --data-urlencode 'query=sum by (name) (changes(container_start_time_seconds{name!=""}[10m]))' | jq .
```
...
```text
inspect logs, then restart affected service if needed
```
